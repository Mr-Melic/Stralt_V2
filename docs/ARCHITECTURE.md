...
`mops.toml` `[canisters.backend.migrations] chain = "src/backend/migrations"`, `check-limit = 6`. Caffeine builds the backend with exactly `mops build` (`src/backend/caffeine.toml` `[build]`), so this chain is what the replica runs. Baseline `.old/src/backend/dist/backend.most` (directory gitignored; file force-tracked) is **Caffeine-owned**: the byte-identical `.most` of the last build Caffeine deployed successfully — the 2026-08-31 import (PR #181 merge `f8aa05e`), latest applied `20260831_000000`, 42 stables, **no GameKey**. Caffeine compares every import against its own copy of that file (PR #311 committed a hand-made `.old` with GameKey and Caffeine still failed M0263), so the repo copy exists only to make local `mops check` behave like Caffeine's. Other recorded shapes: `src/backend/migrations/snapshots/deployed/*.most`; documented non-upgradable shapes: `snapshots/unsupported/` (PR #258 GameKey-on-`20260831`); fresh import: `snapshots/empty-canister.most` (see `snapshots/README.md`). `python3 scripts/check-eop-stables.py` freezes NewActor field lists through 20260901, requires every recorded tail name to remain a chain file, rejects a blank or unrecorded `.old`, and applies the runtime upgrade rule to every snapshot (`--verdict <file.most>`).

Runtime rule (moc 1.11.2 `--enhanced-migration`, `src/lowering/desugar.ml`): the canister stores the names of the migrations it applied; on upgrade the new program matches the **most recently applied name** against its chain file names, loads the state as the chain type at that position and runs every later file. Name missing from the chain → `ICStableRead` of genesis OldActor (`20260801`, optional 37-field #340 shape). A `{}` genesis on a populated Version 1.0.0 heap traps `RTS error: Memory-incompatible program upgrade` (IC0503). Extra fields at the matched position → IC0503. More fields required than the heap holds → `stable variable … not found in persisted state`. Compile-time rule (`mops check-stable`, `mo_types/type.ml`): position = last name in the previous `.most` chain block; every field not produced by a later file is a required input there (M0263), a field nobody consumes is dropped (M0169); at the chain head nothing is required (false negative). The wasm never depends on `.old`.

Lex order (never rename or delete a file; never edit a shipped `NewActor`):

| Module | Job |
| :--- | :--- |
| `20260801_000000.mo` | Genesis (must stay first). `OldActor` is the 37-field #340 actor with every field `?T` so an empty install defaults and a Version 1.0.0 populated heap (Stralt_V2 `ozvtz…` if it never recorded EM names) is a subtype and is copied. `NewActor` is frozen (pre-transient-drop). |
...
| `20260826_000000.mo` | Name-only no-op. Stralt_V2's first Caffeine source export used this genesis name; keep it so a canister that recorded it still matches. |
...
| `20260831_000000.mo` | **Frozen — the 2026-08-31 deployed tail.** Seeds `isSummon` / `summonAI` / `summonLifespan` / `summonUnitDef` on persisted admin `SpellConfig` rows; introduces rollback stables (`*Prev` / `has*Prev`) and `adminAuditLog`. **No GameKey** — PR #258 adding it here (and #311 keeping it here) is what broke the Sep-1 deploy and the #311 import (M0263). |
| `20260901_000000.mo` | **Frozen — GameKey.** `OldActor = {}`; `NewActor` = `gameKeyRequests`, `gameKeyLedger`, `gameKeyReveals`, `lastGameKeyRequestAt`, `nextGameKeyRequestId` with empty / zero defaults. Runs on the Aug-31 tail; a #259–#311 fresh install already recorded this name and is at the head. |
...
- New persistent `let`/`var` on `main.mo` require a **new later** chain file (`20260902+`, input `{}` for pure additions). Static `mops check-stable` is necessary but not sufficient (it passed Aug-31 → #258, which trapped); `scripts/eop-upgrade-matrix.mjs` installs each candidate on PocketIC and upgrades to HEAD.
